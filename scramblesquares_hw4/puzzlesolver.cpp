// Authors: Brogan Clements, Dominick DiMaggio, Ishaan Patel
...
inline int is_left(char *buf1, char *buf2) {
    return buf1[7] == '9' || buf2[3] == '9' ||
           buf1[6] == buf2[2] && buf1[7] != buf2[3];
}

// Is buf2 to the right of buf1
inline int is_right(char *buf1, char *buf2) { return is_left(buf2, buf1); }

// Is buf2 above buf1
inline int is_up(char *buf1, char *buf2) {
    return buf1[1] == '9' || buf2[5] == '9' ||
           buf1[0] == buf2[4] && buf1[1] != buf2[5];
}

// Is buf2 below buf1
inline int is_down(char *buf1, char *buf2) { return is_up(buf2, buf1); }

int is_valid_board(char *board[11]) {
    char *center = board[4];

    // Compare center to adjacencies
    if (!is_up(center, board[1]) || !is_down(center, board[7]) ||
        !is_left(center, board[3]) || !is_right(center, board[5]))
        return 0;
...
int num_from_addr(char *addr) { return addr[8] + 1; }

void print_board(char *board[9],
                 char squares[NUM_ROTATIONS][NUM_LINES + 1][11]) {
    // Checks in order of Top, LR, Bottom
    char(*squares1)[11] = squares[0], (*squares2)[11] = squares[1],
    (*squares3)[11] = squares[2], (*squares4)[11] = squares[3];

    for (int i = 0; i < 7; i += 3) {
        std::cout << "+--------+--------+--------+\n";
        int num1 = num_from_addr(board[i]), num2 = num_from_addr(board[i + 1]),
            num3 = num_from_addr(board[i + 2]);

        std::cout << "|" << num1 << "  " << board[i][0] << board[i][1] << "   |"
                  << num2 << "  " << board[i + 1][0] << board[i + 1][1]
                  << "   |" << num3 << "  " << board[i + 2][0]
                  << board[i + 2][1] << "   |\n";
        std::cout << "|" << board[i][6] << board[i][7] << "    " << board[i][2]
                  << board[i][3] << "|" << board[i + 1][6] << board[i + 1][7]
                  << "    " << board[i + 1][2] << board[i + 1][3] << "|"
                  << board[i + 2][6] << board[i + 2][7] << "    "
                  << board[i + 2][2] << board[i + 2][3] << "|\n";
        std::cout << "|   " << board[i][4] << board[i][5] << "   |   "
                  << board[i + 1][4] << board[i + 1][5] << "   |   "
                  << board[i + 2][4] << board[i + 2][5] << "   |\n";
    }
    std::cout << "+--------+--------+--------+\n";
}
...
void solve(char *board[9], std::vector<bool> used, const int index,
           char squares[NUM_ROTATIONS][NUM_LINES + 1][11],
        ...
        if (g_solutions.empty()) {
            ...
            for (char **sol : g_solutions) {
                ...
                if (are_rotations(copy, sol, squares)) {
                    fresh = false;
                    // If the new board is smaller than the solution, replace
...
int main(int argc, char **argv) {
    ...
    // Note: Count may be unreliable if a piece has both a start and end of the
    // same type
    for (int i = 0; i < NUM_LINES; i++) {
        for (int j = 0; j < 8; j += 2) {
